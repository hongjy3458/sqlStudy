...
SELECT TO_CHAR(SYSDATE, 'AM HH:MI:SS') FROM DUAL; -- AM이나 PM 어떤걸 써도 상관X 그냥 표시만하라는거
...
SELECT 
    EMP_NAME
    , DEPT_CODE
    , DECODE(DEPT_CODE, 'D1', '개발1팀', 'D2', '개발2팀', 'D2', '개발2팀', 'D2'
    , 'D3', '개발3팀'
    , 'D4', '개발4팀'
    , 'D5', '영업1팀'
    , 'D6', '영업2팀') AS 부서명
...
SELECT
    ...
    , JOB_CODE
    , CASE 
        WHEN JOB_CODE = 'J1' THEN '일장'
        WHEN JOB_CODE = 'J2' THEN '이장'
        WHEN JOB_CODE = 'J3' THEN '삼장'
        WHEN JOB_CODE = 'J4' THEN '사장'
        WHEN JOB_CODE = 'J5' THEN '오장'
        ELSE '해당사항없음'
        END AS 직급명
FROM EMPLOYEE;

-- 사원명, 월급, 초급/중급/고급/특급 구분 (급여가 300 까지는 초급, 500 까지는 중급
-- 700까지는 고급, 그 외에는 특급)
...
SELECT
    ...
    , SALARY
    , CASE
        WHEN SALARY > 0 AND SALARY <= 3000000 THEN '초급'
        WHEN SALARY > 3000000 AND SALARY <= 5000000 THEN '중급'
        WHEN SALARY > 5000000 AND SALARY <= 7000000 THEN '고급'
        WHEN SALARY > 7000000 THEN '특급'
        END AS 연봉수준
FROM EMPLOYEE
